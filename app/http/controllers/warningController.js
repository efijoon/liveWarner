...
class DashboardController extends controller {
  ...
  async namadSearch(req, res) {
    ...
    if (namad) {
      ...
      return res
        .status(400)
        .send("نمادی یافت نشد. لطفا نام یک نماد معتبر را وارد کنید.");
    }
  }
  ...
  async makeWarning(req, res) {
    ...
    res.redirect("/warnings/createWarning");
  }
  ...
  async updateWarning(req, res) {
    ...
    await Warning.findByIdAndUpdate(req.params.id, {
      ...req.body,
      compareNumber: parseFloat(req.body.compareNumber),
      symbolID: namad.namadID,
    });
  ...
  async deleteWarning(req, res) {
    ...
    const warningsLength = (
      await Warning.find({ user: req.user._id, sent: false })
    ).length;
  ...
  async validateWarning(req, body, res) {
    ...
    const validateResult = this.compareValidator(
      this.fixNumber(symbol.data[this.convertCompareField(compareField)]),
      comparator,
      compareNumber
    );
    ...
    if (validateResult) {
      this.alertAndBack(req, res, {
        title:
          "شرطی که شما تعیین کردید. هم اکنون برقرار است و نیاز به فعال شدن ندارد.لطفا شرط دیگری را انتخاب کنید",
        type: "error",
        position: "center",
        toast: true,
        showConfirmButton: false,
